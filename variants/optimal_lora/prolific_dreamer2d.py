...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--buffer_size', type=int, default=100, help='buffer size for optimal phi model')
    ...
    assert args.phi_model in ['lora', 'unet_simple', 'optimal']
...
def main():
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            if args.phi_model == 'optimal':
                # update latents_buffer
                latents_buffer = torch.cat((latents_buffer, latents_vsd.clone().detach()), dim=0)
                # clean_latents = scheduler.step(noise_pred, t, latents_vsd).pred_original_sample
                # if latents_buffer is not None:
                #     latents_buffer = torch.cat((latents_buffer, clean_latents.clone().detach()), dim=0)
                # else:
                #     latents_buffer = clean_latents.clone().detach()
                # only keep the last buffer_size
                latents_buffer = latents_buffer[-args.buffer_size:]
                # latents_buffer = latents.clone().detach()
            torch.cuda.empty_cache()
            ######## Do the gradient for unet_phi!!! #########
            if args.generation_mode == 'vsd' and args.phi_model != 'optimal':
