...
def sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings, t, unet_phi=None, guidance_scale=7.5, \
                        grad_scale=1, cfg_phi=1., generation_mode='sds', phi_model='lora', \
                            cross_attention_kwargs={}, multisteps=1, scheduler=None, lora_v=False, \
                                half_inference = False, grad_scale_phi=1., noisy_ref_latent=None, text_embeddings_ref=None):
    ...
    if generation_mode == 'sds':
        noise_pred_ref = predict_noise0_diffuser(unet, noisy_ref_latent, text_embeddings_ref, t, guidance_scale=cfg_phi, cross_attention_kwargs=unet_cross_attention_kwargs, scheduler=scheduler, half_inference=half_inference)
        # SDS
        # grad = grad_scale * (noise_pred - noise)
        grad = grad_scale * (noise_pred - noise_pred_ref)
        # grad = grad_scale * (noise_pred)
        noise_pred_phi = noise_pred_ref
