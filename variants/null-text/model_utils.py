...
def sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings, t, unet_phi=None, guidance_scale=7.5, \
                        grad_scale=1, cfg_phi=1., generation_mode='sds', phi_model='lora', \
                            cross_attention_kwargs={}, multisteps=1, scheduler=None, lora_v=False, \
                                half_inference = False, grad_scale_phi=1., text_embeddings_null_text=None, \
    ...
    with torch.no_grad():
        ...
        if multisteps >= 1:
            noise_pred, noise_pred_1step = predict_noise0_diffuser_multistep(unet, noisy_latents, text_embeddings, t, guidance_scale=guidance_scale, cross_attention_kwargs=unet_cross_attention_kwargs, scheduler=scheduler, steps=multisteps, eta=0., half_inference=half_inference)
        else:
    ...
    if generation_mode == 'sds':
        ...
        with torch.no_grad():
            if phi_model == 'null_text':
                noise_pred_phi = predict_noise0_diffuser(unet_phi, ref_noisy_latent, text_embeddings_null_text, t, guidance_scale=cfg_phi, cross_attention_kwargs=cross_attention_kwargs, scheduler=scheduler, lora_v=lora_v, half_inference=half_inference)
            else:
                noise_pred_phi = predict_noise0_diffuser(unet_phi, ref_noisy_latent, text_embeddings, t, guidance_scale=cfg_phi, cross_attention_kwargs=cross_attention_kwargs, scheduler=scheduler, lora_v=lora_v, half_inference=half_inference)
        # VSD
        # alphas_cumprod = scheduler.alphas_cumprod.to(
